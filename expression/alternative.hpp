  ...
  namespace Core
  {
    ...
    private:
      ...
      operator|(
        ...
        const Alternative<T1, T2>& more_alts)
      {
    ...
    public:
      ...
      constexpr Alternative(const Alternative& input)
        : base(input)
      {}

      constexpr Alternative(Alternative&& input)
        : base(move(input))
      {}

      constexpr Alternative(const only_type& input)
        : base(input, nullptr)
      {}

      constexpr Alternative(only_type&& input)
        : base(move(input), nullptr)
      {}
    ...
    private:
      ...
      operator|(const Alternative& alts, const Expr<T>& expr)
      {
        ...
        return Alternative<only_type, tail_type>(
          alts.first, tail_type(expr));
      }
      ...
      friend constexpr bool
      operator==(nullptr_t, const Alternative&)
      {
        return false;
      }
      ...
      template<typename T1, typename T2>
      friend constexpr bool
      operator==(const Alternative<T1, T2>& y, const Alternative& x)
      {
        return false;
      }
    ...
    template<typename T>
    constexpr auto
    alt(const Expr<T>& expr)
    {
      return Alternative<T, nullptr_t>(expr);
    }

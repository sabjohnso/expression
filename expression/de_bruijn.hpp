...
namespace Expression
{
  namespace Core
  {
    ...
    private:
      ...
      template<typename I, I Index, typename Body>
      static constexpr bool exec(Type<Abstraction<I, Index, Body>>)
      {
        return aux(type<Body>) == data(true, Index);
      }

      template<typename I, I Index, typename Body>
      static constexpr auto aux(Type<Abstraction<I, Index, Body>>)
      {
        return data(aux(type<Body>) == data(true, Index), Index + 1);
      }

      template<typename T>
      static constexpr auto aux(Type<Value<T>>)
      {
        return data(true, 0);
      }

      template<typename I, I Index>
      static constexpr auto aux(Type<Variable<I, Index>>)
      {
        return data(true, 0);
      }

      template<typename F, typename T>
      static constexpr auto aux(Type<Application<F, T>>)
      {
      ...
      {
        return data(
          resf.first && resx.first, max(resf.second, resx.second));
      }
    } is_de_bruijn{};

    /** Rewrite an expression with De Bruijn indexing
     */
    constexpr class DeBruijn
    {
    ...
    public:
      ...
      {
        return exec(
          bool_constant<has_free_variables(type<decay_t<E>>)>{},
          forward<E>(expr));
      }
    ...
    private:
      ...
      public:
        ...
        constexpr Data(const expression_type& input)
          : expr(input)
        {}
      ...
      static constexpr auto
      aux_app(
        const Data<F, depth_fun, repeat_fun>& data_fun,
        const Data<A, depth_arg, repeat_arg>& data_arg)
      {
        return Data < Application<F, A>, max(depth_fun, depth_arg),
               repeat_fun ||
                 repeat_arg >
      ...
      static constexpr auto
      aux_fun(
        const Abstraction<T, Index, Body>& expr,
        const Data<NewBody, Depth, Repeat>& data)
      {
        return aux_fun_index_check(
          bool_constant<Index == Depth>{}, expr, data);
      }
      ...
      aux_fun_index_check(
        true_type,
        const Abstraction<T, Index, Body>&,
        const Data<NewBody, Depth, Repeat>& data)
      {
        static_assert(Index == Depth);
        return Data<Abstraction<T, Index, NewBody>, Index + 1, Repeat>(
          fn(Variable<T, Index>{}, expression(data)));
      }
        ...
        const Data<NewBody, Depth, Repeat>& data)
      {
        ...
        return aux_fun_occurs_check(
          bool_constant<occurs_free(
            integer<T, Depth>, type<NewBody>)>{},
          expr,
          data);
      }
      ...
      template<
        ...
        bool Repeat>
      static constexpr auto
      aux_fun_occurs_check(
        true_type,
        const Abstraction<T, Index, Body>& expr,
        const Data<NewBody, Depth, Repeat>& data)
      {
        return aux_fun(
          expr, Data<NewBody, Depth + 1, true>(expression(data)));
      }
      ...
      static constexpr auto
      aux_fun2(
        const Abstraction<T, Index, OldBody>&,
    ...
    private:
      ...
      static constexpr auto
      aux(const Variable<I, Index>&, Nat<N>)
      {
        return Variable<I, N - Index>();
      }
